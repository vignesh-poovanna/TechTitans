...
import {
  ...
  ingestFlights,
  ingestVessels,
  /* ingestEarthquakes, */ detectGeoConvergence,
...
export interface DataLoaderCallbacks {
  ...
  renderCriticalBanner: (_postures: TheaterPostureSummary[]) => void;
...
export class DataLoaderManager implements AppModule {
  ...
  private boundMarketWatchlistHandler: EventListener | null = null;
  ...
  async loadAllData(): Promise<void> {
    ...
    if (SITE_VARIANT !== 'happy' && this.ctx.mapLayers.ais)
  ...
  private async loadNewsCategory(
    ...
    try {
      ...
      if (digest?.categories && category in digest.categories) {
        ...
        ingestHeadlines(
          items.map((i) => ({ title: i.title, pubDate: i.pubDate, source: i.source, link: i.link }))
        );
  ...
  async loadNews(): Promise<void> {
    ...
    updateAndCheck([{ type: 'news', region: 'global', count: collectedNews.length }])
      .then((anomalies) => {
  ...
  async loadPredictions(): Promise<void> {
    try {
      ...
      const hydrated = getHydratedData('predictions') as
        | { geopolitical?: PredictionMarket[]; tech?: PredictionMarket[] }
        | undefined;
      if (hydrated) {
        const bootstrapMarkets =
          (SITE_VARIANT === 'tech' ? hydrated.tech : hydrated.geopolitical) ?? [];
        if (bootstrapMarkets.length > 0) {
          ...
          this.ctx.statusPanel?.updateFeed('Polymarket', {
            status: 'ok',
            itemCount: bootstrapMarkets.length,
          });
  ...
  async loadNatural(): Promise<void> {
    // Disaster regions (earthquakes, floods, etc.) removed from the map
    this.ctx.intelligenceCache.earthquakes = [];
    this.ctx.map?.setEarthquakes([]);
    this.ctx.map?.setNaturalEvents([]);
    this.ctx.map?.setLayerReady('natural', false);
  }
  ...
  async loadFirmsData(): Promise<void> {
    try {
      ...
      if (totalCount > 0) {
        ...
        updateAndCheck([{ type: 'satellite_fires', region: 'global', count: totalCount }])
          .then((anomalies) => {
            if (anomalies.length > 0) {
              signalAggregator.ingestTemporalAnomalies(anomalies);
              ingestTemporalAnomaliesForCII(anomalies);
  ...
  async loadSecurityAdvisories(): Promise<void> {
    try {
      ...
      if (result.ok) {
        (this.ctx.panels['security-advisories'] as SecurityAdvisoriesPanel)?.setData(
          result.advisories
        );
