...
export class PanelLayoutManager implements AppModule {
  ...
  private scenarioPanel: ScenarioPanel | null = null;
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      <div class="header">
        ...
        <div class="header-right">
          ${SITE_VARIANT === 'full' ? '<button class="scenario-btn" id="scenarioBtn" title="What-If Scenarios">What-If</button>' : ''}
    ...
    // Wire up What-If Scenario button
    if (SITE_VARIANT === 'full') {
      this.scenarioPanel = new ScenarioPanel(() => this.ctx.map);
      document.getElementById('scenarioBtn')?.addEventListener('click', () => {
        this.scenarioPanel?.open();
      });
    }
  }
  ...
  renderCriticalBanner(postures: TheaterPostureSummary[]): void {
    ...
    this.criticalBannerEl.innerHTML = `
      <div class="banner-content">
        <span class="banner-icon">${isCritical ? 'ALERT' : 'WARNING'}</span>
  ...
  private applyTimeRangeFilterToNewsPanels(): void {
    Object.entries(this.ctx.newsByCategory).forEach(([category, items]) => {
      ...
      if (filtered.length === 0 && items.length > 0) {
        // Fallback: show all items instead of empty state
